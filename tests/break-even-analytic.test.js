...
function compareScan(p,label){const scan=E.breakEven(p),exact=E.breakEvenAnalytic(p);assert.equal(exact.length,scan.length);for(let i=0;i<scan.length;i++){if(scan[i].threshold!==null){metrics.scanSwitches++;assert.notEqual(exact[i].threshold,null,`missing exact switch ${label}/${i}`);assert(exact[i].distance<=scan[i].distance+.500001,`exact switch farther than grid ${label}/${i}: ${exact[i].distance} > ${scan[i].distance}`);metrics.maxGridGap=Math.max(metrics.maxGridGap,Math.abs(exact[i].threshold-scan[i].threshold));}if(exact[i].threshold!==null){metrics.exactSwitches++;assert(exact[i].threshold>0&&exact[i].threshold<100);if(scan[i].threshold===null)metrics.exactOnlySwitches++;}}}
