...
test('Degenerate current 0/100 weights safely fall back to the established scan',()=>{const p=twoWay(false);p.criteria[0].weight=0;p.criteria[1].weight=100;assert.deepEqual(E.breakEvenAnalytic(p),E.breakEven(p));});
test('Current leader tie safely falls back to the established scan',()=>{const p=twoWay(false);p.criteria[0].weight=50;p.criteria[1].weight=50;assert.deepEqual(E.breakEvenAnalytic(p),E.breakEven(p));});

let seed=9173;const rnd=()=>((seed=(Math.imul(seed,1664525)+1013904223)>>>0)/4294967296);
function randomProject(optionCount,criterionCount){
 ...
 // Correct floating residue exactly enough for the model's 1e-6 tolerance.
 p.criteria[p.criteria.length-1].weight+=100-p.criteria.reduce((a,c)=>a+c.weight,0);
 p.options=Array.from({length:optionCount},(_,i)=>option('o'+i,'Option '+i,Object.fromEntries(p.criteria.map(c=>[c.id,Math.round(rnd()*10000)/100]))));
 return p;
}
test('Random complete projects never miss a switch found by the 0.5% scan',()=>{for(let n=0;n<250;n++){const p=randomProject(2+Math.floor(rnd()*9),2+Math.floor(rnd()*5)),scan=E.breakEven(p),exact=E.breakEvenAnalytic(p);assert.equal(exact.length,scan.length);for(let i=0;i<scan.length;i++){if(scan[i].threshold===null)continue;assert.notEqual(exact[i].threshold,null,`missing exact switch case ${n}/${i}`);assert(exact[i].distance<=scan[i].distance+.500001,`exact switch farther than grid case ${n}/${i}: ${exact[i].distance} > ${scan[i].distance}`);assert(exact[i].threshold>0&&exact[i].threshold<100);}}});
...
test('Analytical solver is materially faster on a 20×12 complete project',()=>{const p=benchProject();E.breakEven(p);E.breakEvenAnalytic(p);let t=performance.now();for(let i=0;i<4;i++)E.breakEven(p);const scanMs=performance.now()-t;t=performance.now();for(let i=0;i<20;i++)E.breakEvenAnalytic(p);const exactMs=(performance.now()-t)/5; // normalize to four calls
 results.push({name:'benchmark 20x12',pass:true,scanMs:+scanMs.toFixed(3),analyticEquivalent4CallsMs:+exactMs.toFixed(3)});assert(exactMs<scanMs,`analytic ${exactMs}ms was not faster than scan ${scanMs}ms`);});

const summary={suite:'break-even-analytic',tests:results.filter(x=>!x.name.startsWith('benchmark')).length,passed:results.filter(x=>x.pass&&!x.name.startsWith('benchmark')).length,failed:results.filter(x=>!x.pass).length,results};
fs.mkdirSync('qa',{recursive:true});fs.writeFileSync('qa/break-even-analytic-results.json',JSON.stringify(summary,null,2));
console.log(JSON.stringify({suite:summary.suite,tests:summary.tests,passed:summary.passed,failed:summary.failed,benchmark:results.find(x=>x.name==='benchmark 20x12')}));if(summary.failed)process.exitCode=1;
