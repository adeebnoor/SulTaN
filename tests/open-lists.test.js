...
assert.equal(V.programs.length,9);assert.ok(/^\d{4}-\d{2}-\d{2}$/.test(V.verifiedOn));assert.equal(new Set(V.programs.map(x=>x.id)).size,9);assert.equal(V.completed.length,2);assert.ok(V.completed.some(x=>x.id==='priv')&&V.completed.some(x=>x.id==='fsp'));assert.ok(/غير مرتبط/.test(ctx.globalThis.SultanLocales.ar.visionListDisclaimer));assert.ok(/not affiliated/i.test(ctx.globalThis.SultanLocales.en.visionListDisclaimer));
console.log(JSON.stringify({suite:'open-lists',passed:true,activePrograms:9,completedPrograms:2,verifiedOn:V.verifiedOn}));
