...
function fullDocument(body,title){const style=`body{font:16px/1.8 "Segoe UI",Tahoma,Arial,sans-serif;color:#12203f;margin:28px auto;max-width:1100px;padding:24px}h1,h2,h3{color:#0b2d63}section,dl{border-bottom:1px solid #ddd;padding-bottom:14px}dt{font-weight:bold}dd{margin:0 0 10px;white-space:pre-wrap}table{width:100%;border-collapse:collapse;margin:12px 0}td,th{border:1px solid #ddd;padding:8px;text-align:start;vertical-align:top}header{border-bottom:3px solid #caa85e}.report-extra{margin-top:28px}@media print{body{margin:0;font-size:11px}h2,h3{break-after:avoid}tr{break-inside:avoid}.noPrint{display:none}}`;return `<!doctype html><html lang="${SultanI18n.language}" dir="${SultanI18n.direction}"><meta charset="utf-8"><title>${esc(title)}</title><style>${style}</style><body>${body}</body></html>`;}
...
function printInternal(){
 ...
 w.opener=null;w.document.open();w.document.write(fullDocument(root.SultanApp.getReport(),T('internalReport')));w.document.close();w.focus();w.print();
}
// The sole strategy action exports the complete internal report, never the legacy partial draft.
document.addEventListener('click',e=>{
 const b=e.target.closest('[data-action="print"]');
