...
function extensionReport(p){
 ...
 p.options.forEach(o=>{
  if(o.type==='divest')chunks.push(`<h3>${esc(o.title)} · ${esc(T('divest'))}</h3><dl><dt>${esc(T('divestStop'))}</dt><dd>${esc(o.divestStop)}</dd><dt>${esc(T('divestResources'))}</dt><dd>${fmt(o.releasedResources)}</dd><dt>${esc(T('divestRedeploy'))}</dt><dd>${esc(o.redeployTo)}</dd><dt>${esc(T('divestEvidence'))}</dt><dd>${esc(o.divestEvidence)}</dd><dt>${esc(T('divestImpact'))}</dt><dd>${esc(o.divestImpact)}</dd></dl>`);
  if(o.type==='moonshot'&&text(o.stopEvidence))chunks.push(`<h3>${esc(o.title)} · ${esc(T('moonshotStopEvidence'))}</h3><p>${esc(o.stopEvidence)}</p>`);
  if(o.type!=='requirement'&&(text(o.riskSource)||text(o.riskDate)))chunks.push(`<h3>${esc(o.title)} · ${esc(T('riskSource'))}</h3><p>${esc(o.riskDate||'—')} → ${esc(o.riskSource||'—')}</p>`);
  if(Array.isArray(o.assumptions)&&o.assumptions.length){const rows=o.assumptions.map(a=>`<dl><dt>${esc(T('assumptionText'))}</dt><dd>${esc(a.text)}</dd><dt>${esc(T('assumptionDuration'))}</dt><dd>${esc(a.expectedPersistence)}</dd><dt>${esc(T('assumptionOwner'))}</dt><dd>${esc(a.owner)}</dd><dt>${esc(T('assumptionTestDate'))}</dt><dd>${esc(a.testDate)}</dd><dt>${esc(T('assumptionEvidence'))}</dt><dd>${esc(a.testEvidence)}</dd><dt>${esc(T('assumptionFailure'))}</dt><dd>${esc(a.failureImpact)}</dd></dl>`).join('');chunks.push(`<h3>${esc(o.title)} · ${esc(T('assumptionRegister2'))}</h3>${rows}`);}
 });
 p.transitions.filter(t=>text(t.maturityFamily)).forEach(t=>chunks.push(`<h3>${esc(t.domain)} · ${esc(T('maturityFamily'))}</h3><p>${esc(t.maturityFamily)}</p>`));
 p.initiatives.forEach(i=>{const gated=(i.budget||[]).filter(b=>text(b.releaseEvidence));if(gated.length)chunks.push(`<h3>${esc(i.title)} · ${esc(T('gatedFundingTitle'))}</h3><table><thead><tr><th>${esc(T('fundingYear'))}</th><th>${esc(T('releaseEvidence'))}</th></tr></thead><tbody>${gated.map(b=>`<tr><td>${b.year}</td><td>${esc(b.releaseEvidence)}</td></tr>`).join('')}</tbody></table>`);});
...
function renderSemanticHints(){if(section()!=='review'||!E.semanticIssues)return;const content=document.getElementById('content');if(!content)return;content.querySelector('.semantic-hints')?.remove();const rows=E.semanticIssues(root.SultanApp.getProject());if(!rows.length)return;const c=document.createElement('article');c.className='card semantic-hints';c.innerHTML=`<h2>${esc(T('semanticHintsTitle'))}</h2><p class="hint">${esc(T('semanticHintsNote'))}</p>${rows.map(r=>`<p class="semantic-hint"><b>${esc(r.rule)}</b> · ${esc(r.message)}</p>`).join('')}`;const d=content.querySelector('.review-executive');if(d)d.insertAdjacentElement('afterend',c);else content.prepend(c);}
...
function afterRender(){setTimeout(()=>{renderVersion();renderRecovery();renderIdentity();renderChoices();renderReferences();renderRoadmap();renderCollaboration();renderDashboard();renderExecutive();renderSemanticHints();renderExport();releasedResources();const c=document.querySelector('.break-even-card');if(c)c.__finalProject=root.SultanApp.getProject();},0);}
